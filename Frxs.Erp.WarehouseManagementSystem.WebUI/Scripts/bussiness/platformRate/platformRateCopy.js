...
function init() {
    ...
    $.ajax({
        url: "../PlatformRate/GetPlatformRateCopy",
        type: "post",
        data: { id: id },
        dataType: 'json',
...
function initProductGrid(griddata) {
    $('#gridProduct').datagrid({
        ...
        columns: [[
            ...
            { title: '库存单位配送价格', field: 'SalePrice', width: 150, align: 'left' },
            { title: '国际条码', field: 'BarCode', width: 150, hidden: true },
            //{ title: '原平台费率', field: 'Point', width: 100 }
            { title: '原平台费率', field: 'OldPoint', width: 100 }
            ,{
                title: '平台费率', field: 'Point', width: 100
                , editor: {
                    type: 'numberbox',
                    min: 0,
                    max: 1,
...
function getProductCheck() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (point == 0) {
            $.messager.alert("提示", "平台费率不能为0！", "info");
            return false;
        }else if (point < 0 || point > 1) {
            $.messager.alert("提示", "平台费率只能填小于或等于1的小数！", "info");
            return false;
        }
    }
    return true;
