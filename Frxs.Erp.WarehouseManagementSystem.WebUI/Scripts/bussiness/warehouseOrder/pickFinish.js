...
function loadgrid() {
    $('#grid').datagrid({
        ...
        //    //加载成功后让某一列处于编辑状态
        //    $(this).datagrid('ColumnEditing', 'PickQty');
        //},
        ...
        columns: [[
            ...
            {
                title: '单位', field: 'SaleUnit', width: 50, align: 'center', editor: {
                    type: 'combobox',
                    options: {
                        ...
                        onChange: function (value) {
                            ...
                            if (!isNaN(value)) {
                                ...
                                var oldPreQty = row.PreQty;

                                //计算数量
                                var Qty = parseFloat(parseFloat(oldSalePackingQty) * parseFloat(oldPreQty) / parseFloat(value)).toFixed(2);

                                //计算订单数量
                                row.PreQty = Qty;

                                //计算数量
                                row.PickQty = parseFloat(parseFloat(oldSalePackingQty) * parseFloat(row.PickQty) / parseFloat(value)).toFixed(2);
            ...
            { title: '订单数量', field: 'PreQty', width: 150, align: 'right' },
            {
                title: frxs.setTitleColor('数量'), field: 'PickQty', align: 'right', width: 150, editor: {
                    type: 'numberbox',
                    options: {
                        precision: 2,
                        min: 0
...
function saveData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].PickQty == null || rows[i].PickQty == undefined || rows[i].PickQty.toString()=="") {
            window.top.$.messager.alert("提示", "第 " + (i + 1) + " 行商品未输入拣货数量，不能提交！", "info");
            return false;
        }
    }

    window.parent.$(window.frameElement).closest(".window").find("#btnPickFinishOk").addClass("easyui-linkbutton").linkbutton('disable');
    
    var loading = frxs.loading("正在加载中，请稍后...");
    
    var products = JSON.stringify(rows);
    $.ajax({
        url: "../WarehouseOrder/PickFinishHandle",
        type: "post",
        dataType: "json",
        data: {
            OrderID: orderid,
        ...
        success: function (result) {
            loading.close();
            window.parent.$(window.frameElement).closest(".window").find("#btnPickFinishOk").addClass("easyui-linkbutton").linkbutton('enable');
            if (result != undefined && result.Info != undefined) {
                if (result.Flag == "SUCCESS") {
                    window.top.$.messager.alert("提示", "拣货成功", "info", function () {
...
function totalCalculate() {
    ...
    var totalPreQty = 0.00;
    for (var i = 0; i < rows.length; i++) {
        var pickQty = parseFloat(rows[i].PickQty?rows[i].PickQty:0);
        var preQty = parseFloat(rows[i].PreQty?rows[i].PreQty:0);
        totalPickQty += pickQty;
        totalPreQty += preQty;
    }

    $('#grid').datagrid('reloadFooter', [
       { "PickQty": "拣货数总计：" + parseFloat(totalPickQty).toFixed(2), "PreQty": "总计：" + parseFloat(totalPreQty).toFixed(2) }
    ]);
}
