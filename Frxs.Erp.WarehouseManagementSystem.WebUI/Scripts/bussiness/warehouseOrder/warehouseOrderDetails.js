...
function loadgrid(griddata) {
    $('#grid').datagrid({
        ...
        columns: [[
            ...
            {
                title: '数量', field: 'SaleQty', width: 80, align: 'right', editor: {
                    type: 'numberbox',
                    options: {
                        precision: 2
                    }
            ...
            {
                title: '配送价', field: 'SalePrice', width: 100, align: 'right'
            },
            {
                title: '平台费率', field: 'ShopAddPerc', width: 70, align: 'right'
            },
            { title: '金额', field: 'SubAmt', width: 160, align: 'right' },
            {
...
function saveData() {
    ...
    if (count == 0) {
        loading.close();
        $.messager.alert("提示", "批量改单商品数据不能为空！", "info");
        return false;
    }
    ...
    if (countOrder == 0) {
        loading.close();
        $.messager.alert("提示", "批量改单订单数据不能为空！", "info");
        return false;
    }
    ...
    jsonEditOrder += "\"Remark\":'" + $("#Remark").val() + "'";
    jsonEditOrder += "}";
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.Flag == "SUCCESS") {
                ...
                frxs.updateTabTitle("查看商品批量推荐单号" + result.Data.EditID, "icon-search");
