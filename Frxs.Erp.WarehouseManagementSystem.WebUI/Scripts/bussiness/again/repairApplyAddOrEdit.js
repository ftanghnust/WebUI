...
function loadgrid(griddata) {
    $('#grid').datagrid({
        ...
        columns: [[
            {
            title: frxs.setTitleColor('单位'), field: 'Unit', align: 'left', width: 80,align: 'center', editor: {
                    type: 'combobox',
                    options: {
            ...
            { title: frxs.setTitleColor('补货数量'), field: 'AppQty', width: 120,align: 'right', editor: 'text' },
            { title: '补货价格', field: 'Price', width: 120, align: 'right'},
            { title: '补货金额', field: 'SubAmt', width: 120, align: 'right'},
...
function saveData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].SKU) {
            if (rows[i].AppQty <= 0) {
                message = "商品：" + rows[i].ProductName + " 补货数量不能等于 0<br/>";
...
function showSearchProduct(parm) {
    frxs.dialog({
        title: "选择商品资料",
        url: "../BuyOrderPre/SearchBuyProduct?vendorId=0&productCode=" + parm + "&SubWID=" + $("#SubWID").combobox('getValue'),
        owdoc: window.top,
        width: 850,
        height: 500
    });
...
function add() {
    //修改标题
    frxs.updateTabTitle("添加补货申请单", "icon-add");
    location.href = '../Again/RepairApplyAddOrEdit';
}
...
function edit() {
    ...
    frxs.updateTabTitle("编辑补货申请单" + $("#AppID").val(), "icon-edit");
    //location.href = '../Again/RepairApplyAddOrEdit?appId=' + $("#AppID").val();
}
...
function print() {
    ...
    if (appId != "") {
        var thisdlg = frxs.dialog({
            title: "打印补货申请单",
            url: "../FastReportTemplets/Aspx/PrintRepairApply.aspx?AppID=" + appId,
            owdoc: window.top,
            width: 765,
            height: 600
        });
...
function exportData() {
    ...
    location.href = "../Again/ExportExcelAgain?appId=" + appId+"&type=1";
}
