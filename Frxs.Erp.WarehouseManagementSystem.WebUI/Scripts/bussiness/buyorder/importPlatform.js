...
function gridload(){
    $('#grid').datagrid({
        ...
        height: 430,
        methord: 'post',                    //提交方式
        //url: '../ProductList/GetImportData',//Aajx地址
        sortName: 'ID',                 //排序字段
        sortOrder: 'desc',                  //排序方式
        idField: 'ID',       //主键
        pageSize: 5000,                       //每页条数
        ...
        columns: [[
            ...
            {
                title: '商品编码', field: 'SKU', width: 80, formatter: function (value) {
                    if (!value) {
            ...
            { title: '包装单位', field: 'Unit', width: 80, align: 'center' },
            { title: '原门店积分', field: 'ShopPoint', width: 100,formatter:function (value) {
                return parseFloat(value).toFixed(2);
            } },
            {
                title: '新门店积分', field: 'NewShopPoint', width: 140, formatter: function (value) {
            ...
            {
                title: '原绩效分率', field: 'BasePoint', width: 100, formatter: function (value) {
                    return parseFloat(value).toFixed(3);
                }
            },
            {
                title: '新绩效分率', field: 'NewBasePoint', width: 140, formatter: function (value) {
                    if (isNaN(value) || value < 0) {
                        return "<div style='width:100%;background:red'>&nbsp;</div>";
                    }
                    return parseFloat(value).toFixed(3);
            ...
            {
                title: '原物流费率', field: 'VendorPerc1', width: 100, formatter: function (value) {
                    return parseFloat(value).toFixed(3);
                }
            },
            {
                title: '新物流费率', field: 'NewVendorPerc1', width: 140, formatter: function (value) {
                    if (isNaN(value) || value < 0) {
                        return "<div style='width:100%;background:red'>&nbsp;</div>";
                    }
                    return parseFloat(value).toFixed(3);
            ...
            {
                title: '原仓储费率', field: 'VendorPerc2', width: 100, formatter: function (value) {
                    return parseFloat(value).toFixed(3);
                }
            },
            {
                title: '新仓储费率', field: 'NewVendorPerc2', width: 140, formatter: function (value) {
                    if (isNaN(value) || value < 0) {
                        return "<div style='width:100%;background:red'>&nbsp;</div>";
                    }
                    return parseFloat(value).toFixed(3);
            ...
            {
                title: '原平台费率', field: 'ShopAddPerc', width: 100, formatter: function (value) {
                    return parseFloat(value).toFixed(3);
                }
            },
            {
                title: '新平台费率', field: 'NewShopAddPerc', width: 140, formatter: function (value) {
                    if (isNaN(value) || value < 0) {
                        return "<div style='width:100%;background:red'>&nbsp;</div>";
                    }
                    return parseFloat(value).toFixed(3);
            ...
            { title: '国际条码', field: 'BarCode', width: 140 },
...
function importData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (isNaN(rows[i].NewShopPoint) || rows[i].NewShopPoint < 0 || rows[i].NewShopPoint > 10000.00) {
            msg = "序号 " + rows[i].Index + " 的新门店积分应为 0-10000.00 之间";
            break;
        }
        if (isNaN(rows[i].NewBasePoint) || rows[i].NewBasePoint < 0 || rows[i].NewBasePoint>100.000) {
            msg = "序号 " + rows[i].Index + " 的新绩效分率应为 0-100.000 之间";
            break;
        }
        if (isNaN(rows[i].NewVendorPerc1) || rows[i].NewVendorPerc1 < 0 || rows[i].NewVendorPerc1>1.000) {
            msg = "序号 " + rows[i].Index + " 的新物流费率应为 0-1.000 之间";
            break;
        }
        if (isNaN(rows[i].NewVendorPerc2) || rows[i].NewVendorPerc2 < 0 || rows[i].NewVendorPerc2>1.000) {
            msg = "序号 " + rows[i].Index + " 的新仓储费率应为 0-1.000 之间";
            break;
        }
        if (isNaN(rows[i].NewShopAddPerc) || rows[i].NewShopAddPerc < 0 || rows[i].NewShopAddPerc > 1.000) {
            msg = "序号 " + rows[i].Index + " 的新平台费率应为 0-1.000 之间";
            break;
        }
