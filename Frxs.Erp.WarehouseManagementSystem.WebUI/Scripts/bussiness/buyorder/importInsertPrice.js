...
function gridload()
{
    $('#grid').datagrid({
        ...
        columns: [[
            ...
            {
                title: '商品编码', field: 'SKU', width: 60, formatter: function (value) {
                    if (!value) {
            ...
            { title: '原库存单位进货价', field: 'BuyPrice', width: 120 ,formatter:function (value) {
                return parseFloat(value).toFixed(4);
            }},
            {
                title: '新库存单位进货价', field: 'NewPrice', width: 120, formatter: function (value) {
...
function importData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (isNaN(rows[i].NewPrice) || rows[i].NewPrice < 0 || rows[i].NewPrice > 999999.9999) {
            msg = "序号 " + rows[i].Index + " 的新库存单位进货价应为 0-999999.9999 之间";
            break;
        }
