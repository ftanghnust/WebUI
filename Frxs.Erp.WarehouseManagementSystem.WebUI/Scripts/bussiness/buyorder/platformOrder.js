...
function initGrid() {
    $('#grid').datagrid({
        ...
        onDblClickCell: function () {
            ...
            frxs.openNewTab("查看费率积分调整单" + selRow.AdjID, "../InsertPriceChange/PlatformOrderAddOrEdit?action=view&adjId=" + selRow.AdjID, "icon-search");
        },
        queryParams: {
            AdjType: 2,
            //查询条件
...
//添加
function add() {
    frxs.openNewTab("添加费率积分调整单", "../InsertPriceChange/PlatformOrderAddOrEdit", "icon-add", window);
}
...
function look() {
    ...
    frxs.openNewTab("查看费率积分调整单" + selRow[0].AdjID, "../InsertPriceChange/PlatformOrderAddOrEdit?action=view&adjId=" + selRow[0].AdjID, "icon-search");
}
