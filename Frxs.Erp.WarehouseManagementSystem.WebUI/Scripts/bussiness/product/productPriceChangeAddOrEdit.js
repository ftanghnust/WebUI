...
function loadgrid(griddata) {
    $('#grid').datagrid({
        ...
        frozenColumns: [[
            ...
            { title: '商品名称', field: 'ProductName', width: 260, formatter: frxs.replaceCode }
        ]],
        columns: [[
            {
                title: '库存单位', field: 'Unit', width: 180, align: 'center'
            },
            {
                title: '原库存单位配送价', field: 'OldPrice', align: 'right', width: 180, formatter: function (value) {
                    if (value) {
            ...
            {
            title: frxs.setTitleColor('库存单位配送价'), field: 'Price', width: 200, align: 'right', editor: 'text', formatter: function (value) {
                    if (value) {
                        return parseFloat(value).toFixed(4);
                    }
                    return "0.0000";
            ...
            {
                title: '包装数', field: 'PackingQty', width: 180, align: 'center', formatter: function (value) {
                    if (value) {
...
function saveData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        if (rows[i].SKU) {
            if (rows[i].Price < 0 || rows[i].Price > 999999.9999) {
                message = "商品：" + rows[i].ProductName + " 库存单位配送价应为 0-999999.9999 之间<br/>";
    ...
    jsonBuyOrder += "\"AdjType\":\"1\"";     //(0:采购(进货)价;1:配送(批发)价;3:费率及积分)
...
function backFillProduct2(product) {
    ...
    row.OldPrice = parseFloat(product.UnitPrice).toFixed(4);
...
function add() {
    //修改标题
    frxs.updateTabTitle("添加配送价格调整单", "icon-add");
    location.href = '../ProductList/ProductPriceChangeAddOrEdit';
}
...
function edit() {
    //修改标题
    SetPermission("edit");
    frxs.updateTabTitle("编辑配送价格调整单" + $("#AdjID").val(), "icon-edit");
    //location.href = '../ProductList/ProductPriceChangeAddOrEdit?adjId=' + $("#AdjID").val();
}
...
function importData() {
    var thisdlg = frxs.dialog({
        title: "数据导入",
        url: "../ProductList/ImportShippPrice",
        owdoc: window.top,
        width: 825,
        height: 650,
        buttons: [{
...
function print() {
    ...
    if (adjid != "") {
        var thisdlg = frxs.dialog({
            title: "打印配送价调整单",
            url: "../FastReportTemplets/Aspx/PrintPriceChange.aspx?AdjID=" + adjid,
            owdoc: window.top,
            width: 765,
            height: 600
        });
...
function exportData() {
    ...
    location.href = "../ProductList/ExportExcel?ajdId=" + adjId;
}
