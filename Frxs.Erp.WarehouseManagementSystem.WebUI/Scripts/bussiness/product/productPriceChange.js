...
function initGrid() {
    $('#grid').datagrid({
        ...
        onDblClickCell: function () {
            ...
            frxs.openNewTab("查看配送价格调整单"+selRow.AdjID, "../ProductList/ProductPriceChangeAddOrEdit?action=view&adjId=" + selRow.AdjID, "icon-search");
        },
...
//添加
function add() {
    frxs.openNewTab("添加配送价格调整单", "../ProductList/ProductPriceChangeAddOrEdit", "icon-add", window);
}
...
function edit() {
    ...
    if (selRow[0].Status == 2) {
        $.messager.alert("提示", "已过账的单据不能编辑", "info");
        return;
    }
    frxs.openNewTab("编辑配送价格调整单", "../ProductList/ProductPriceChangeAddOrEdit?adjId=" + selRow[0].AdjID, "icon-edit");
}
...
function look() {
    ...
    frxs.openNewTab("查看配送价格调整单" + selRow[0].AdjID, "../ProductList/ProductPriceChangeAddOrEdit?action=view&adjId=" + selRow[0].AdjID, "icon-search");
