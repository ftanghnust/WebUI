...
function gridload() {
    $('#grid').datagrid({
        ...
        columns: [[
            ...
            {
                title: '序号', field: 'Index', width: 50, align: 'left'
            },
            {
            ...
            {
                title: '商品名称', field: 'ProductName', width: 160, formatter: function (value) {
                    if (!value) {
                        return "<div style='width:100%;background:red'>&nbsp;</div>";
                    }
                    return value;
            ...
            { title: '包装单位', field: 'Unit', width: 60, align: 'center' },
            {
                title: '原库存单位配送价', field: 'UnitPrice', width: 120, formatter: function (value) {
                    return parseFloat(value).toFixed(4);
                }
            },
            {
                title: '新库存单位配送价', field: 'NewPrice', width: 120, formatter: function (value) {
...
function importData() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (isNaN(rows[i].NewPrice) || rows[i].NewPrice < 0 || rows[i].NewPrice > 999999.9999) {
            msg = "序号 " + rows[i].Index + " 的新库存单位配送价应为 0-999999.9999 之间";
            break;
        }
        //验证重复数据
