...
function init() {
    ...
    $.ajax({
        url: "../ScorePromotion/GetScorePromotion",
        type: "post",
        data: { id: id },
        dataType: 'json',
...
function initProductGrid(griddata) {
    $('#gridProduct').datagrid({
        ...
        columns: [[
            ...
            { title: '原库存单位积分', field: 'OldPoint', width: 100, align: 'right' },
            {
                title: frxs.setTitleColor('库存单位积分'), field: 'Point', width: 100, align: 'right'
                , editor: {
                    type: 'numberbox',
                    options: {
                        precision: 2,
            ...
            //{
            //    title: frxs.setTitleColor('限购库存单位数量'), field: 'MaxOrderQty', align: 'right', width: 120
            //    , editor: {
            //        type: 'numberbox',
            //        options: {
            //            precision: 2,
            ...
            //},
            {
                title: frxs.setTitleColor('配送单位限购数量'), field: 'MaxOrderQty', align: 'right', width: 120
                , editor: {
                    type: 'numberbox',
                    options: {
                        precision: 2,
...
function initGroupGrid(griddata) {
    $('#gridGroup').datagrid({
        ...
        columns: [[
            ...
            {
                title: '门店类型', field: 'ShopType', width: 90, align: 'center', formatter: function (value, rec) {
                    if (value == "0") {
                        return "加盟店";
                    }
                    else if (value == "1") {
...
function edit() {
    frxs.updateTabTitle("编辑积分促销单" + nosaleId, "icon-edit");
    SetPermission("edit");
}
...
function copy() {
    ...
    $.messager.confirm("提示", "复制该积分促销单？", function (r) {
        if (r) {
            location.href = '../ScorePromotion/ScorePromotionCopy?Id=' + idStr;
            frxs.updateTabTitle("复制积分促销单" + idStr, "icon-add");
            $("#closeBtn").click(function () {
                frxs.tabClose();
            });
        }
...
function getProductCheck() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (point == null) {
            $.messager.alert("提示", "请填写库存单位积分！", "info");
            $('#gridProduct').datagrid('clearSelections');
            onClickRow($('#gridProduct').datagrid('getRowIndex', rows[i]));
            return false;
        } else if (point < 0) {
            $.messager.alert("提示", "库存单位积分不能为负数！", "info");
            $('#gridProduct').datagrid('clearSelections');
            onClickRow($('#gridProduct').datagrid('getRowIndex', rows[i]));
            return false;
        } else if (point > 10000) {
            $.messager.alert("提示", "库存单位积分不能大于10000！", "info");
            $('#gridProduct').datagrid('clearSelections');
            onClickRow($('#gridProduct').datagrid('getRowIndex', rows[i]));
            return false;
        }
        var maxOrderQty = rows[i].MaxOrderQty;
        if (maxOrderQty < 0) {
            $.messager.alert("提示", "限购库存单位数量不能为负数！", "info");
            $('#gridProduct').datagrid('clearSelections');
            onClickRow($('#gridProduct').datagrid('getRowIndex', rows[i]));
            return false;
        }
...
function print() {
    ...
    if (promotionId != "") {
        var thisdlg = frxs.dialog({
            title: "打印门店积分促销单",
            url: "../FastReportTemplets/Aspx/PrintScorePromotion.aspx?PromotionID=" + promotionId,
            owdoc: window.top,
            width: 765,
            height: 600
        });
