...
function initStockAdjDetailGrid() {
    ...
    $('#gridStockAdjDetail').datagrid({
        ...
        showFooter: true,                   //表示统计列（这里统计所有的数据，直接从后台读取过来）
...
function addStockAdjDetail() {
    var thisdlg = frxs.dialog({
        title: "新增盘点明细",
        url: "../StockAdjDetail/StockAdjDetailAddOrEdit",
        owdoc: window.top,
        width: dialogWidth,
        height: dialogHeight,
        buttons: [{
...
function editStockAdjDetail(detailId) {
    var thisdlg = frxs.dialog({
        title: "编辑盘点明细",
        url: "../StockAdjDetail/StockAdjDetailAddOrEdit?Id=" + detailId,
        owdoc: window.top,
        width: dialogWidth,
        height: dialogHeight,
        buttons: [{
                ...
                if (obj.Flag == "SUCCESS") {
                    if (obj.Data > 0) {
                        ...
                        var thisdlg = frxs.dialog({
                            ...
                            url: "../StockAdjDetail/ImportStockAdjDetail?adjType=1",
                            owdoc: window.top,
                            width: 800,
                            height: 260,
                            buttons: [{
...
//新增按钮
function add() {
    location.href = '../StockCheckLoss/StockCheckAddOrEdit';
                ...
                success: function (result) {
                    if (result != undefined && result.Info != undefined) {
                        if (result.Flag == "SUCCESS") {
                            ...
                            //frxs.tabClose();
                            init();
                            $.messager.alert('提示', "确认成功!", 'info');//单条记录的确认成功，不需要给复杂提示。
                        }
                        else {
...
function print() {
    ...
    if (adjId != "") {
                ...
                if (obj != undefined && obj.Info != undefined) {
                    if (obj.Flag == "SUCCESS") {
                        if (obj.Data > 0) {
                            var thisdlg = frxs.dialog({
                                title: "打印库存盘亏单",
                                url: "../FastReportTemplets/Aspx/PrintStockCheckLoss.aspx?AdjID=" + adjId,
                                owdoc: window.top,
                                width: 765,
                                height: 600
                            });
                        } else {
                            $.messager.alert('提示', "盘亏明细不能为空！", 'info');
                            loading.close();
                        }
                    } else {
                        $.messager.alert('提示', obj.Info, 'info');
        ...
        $.messager.alert("提示", "盘亏单号不能为空！", "info");
    }
}
